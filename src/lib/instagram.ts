// src/lib/instagram.ts — Instagram Graph API + mock fallback
...
function toUnified(v: any) {
  return {
    ...
    platformMeta: { label: "Instagram", badge: "IG", color: "#E1306C", bg: "rgba(225,48,108,0.9)" },
  };
}

export function getMockInstagramCatalog() {
  return [
...
export async function searchInstagram(query: string, accessToken?: string): Promise<{ videos: any[]; isMock: boolean; error?: string }> {
  const catalog = getMockInstagramCatalog();
  ...
  // Instagram Graph API — hashtag search requires business account. Attempt generic search; fallback to mock on failure.
  // Example: https://graph.facebook.com/v18.0/ig_hashtag_search?user_id=...&q=...&access_token=...
  // For generic token, try basic display/search emulation
  try {
    // Use Facebook Graph search for media: https://graph.facebook.com/v18.0/me/media?fields=id,caption,media_type,media_url,thumbnail_url,permalink,timestamp&access_token=...
    // This is user-scoped; without known IG user ID, we can't reliably search, so we probe and fallback
    ...
    if (!res.ok) throw new Error(`Instagram Graph ${res.status}`);
    const data: any = await res.json();
    const items = data.data || [];
    if (!Array.isArray(items) || items.length === 0) throw new Error("empty instagram media");
    // Filter to video/reels-like items; use caption search
    let filtered = items;
    if (query) filtered = items.filter((it: any) => (it.caption || "").toLowerCase().includes(qLower));
    if (filtered.length === 0) filtered = items;
    const videos = filtered.slice(0, 12).map((it: any) => toUnified({
      platformId: it.id,
      youtubeId: it.id,
      title: it.caption ? it.caption.slice(0, 80) : "Instagram Reel",
      channelName: "@instagram_creator",
      handle: "@instagram_creator",
      thumbnailUrl: it.thumbnail_url || it.media_url || `https://images.unsplash.com/photo-1534972195531-d756b9bfa9f2?auto=format&fit=crop&w=600&q=80`,
      views: "100K views",
      viewsRaw: 100000,
      duration: "0:22",
      category: "Tech",
    ...
    console.warn("[instagram] real fetch failed, falling back to mock:", e?.message || e);
