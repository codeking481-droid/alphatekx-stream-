// src/lib/youtube.ts — YouTube Data API v3 + Piped + Scrape fallbacks (real, no mock nonsense) + cache
...
const searchCache = new Map<string, { videos: any[]; ts: number }>();
const CACHE_TTL = 5 * 60 * 1000;

async function fetchPipedSearch(query: string): Promise<any[]> {
  try {
    ...
    const videos = items.slice(0, 12).map((it: any) => {
    ...
    console.warn("[youtube] Piped fallback failed", e);
    throw e;
  }
}

async function fetchScrapeSearch(query: string): Promise<any[]> {
  try {
    ...
    // Fetch real titles via oEmbed in parallel for accurate titles (no mock nonsense)
    ...
    console.warn("[youtube] Scrape fallback failed", e);
    throw e;
  }
}

export async function searchYouTube(query: string, apiKey?: string): Promise<{ videos: any[]; isMock: boolean; error?: string }> {
  const cacheKey = (query || "").toLowerCase().trim() || "__empty__";
  const cached = searchCache.get(cacheKey);
  if (cached && Date.now() - cached.ts < CACHE_TTL) {
    return { videos: cached.videos, isMock: false };
  }

  const catalog = getMockYouTubeCatalog();
  const qLower = (query || "").toLowerCase();
  const filter = (arr: any[]) => {
    let f = arr.filter(v => !query || v.title.toLowerCase().includes(qLower) || v.channelName.toLowerCase().includes(qLower));
    if (f.length === 0) f = arr;
    return f;
  };
  ...
  if (!apiKey) {
    try {
      const piped = await fetchPipedSearch(query || "AI");
      searchCache.set(cacheKey, { videos: piped, ts: Date.now() });
      return { videos: piped, isMock: false };
    } catch {
      try {
        const scraped = await fetchScrapeSearch(query || "AI");
  ...
  try {
    ...
    if (!searchRes.ok) {
      ...
      if (searchRes.status === 403 || searchRes.status === 429) {
        try {
          const piped = await fetchPipedSearch(query || "AI");
          searchCache.set(cacheKey, { videos: piped, ts: Date.now() });
          return { videos: piped, isMock: false, error: `YouTube ${searchRes.status} — Piped real fallback` };
        } catch {}
        try {
          const scraped = await fetchScrapeSearch(query || "AI");
          searchCache.set(cacheKey, { videos: scraped, ts: Date.now() });
    ...
    try {
      ...
      if (detailsRes.ok) {
        ...
        (detailsData.items || []).forEach((it: any) => {
          detailsMap[it.id] = { duration: parseIsoDuration(it.contentDetails?.duration), views: formatViews(it.statistics?.viewCount), viewsRaw: parseInt(it.statistics?.viewCount || "0") };
        });
      }
    ...
    try {
      const piped = await fetchPipedSearch(query || "AI");
      searchCache.set(cacheKey, { videos: piped, ts: Date.now() });
      return { videos: piped, isMock: false, error: e.message + " — Piped fallback" };
    } catch {}
    try {
      const scraped = await fetchScrapeSearch(query || "AI");
      searchCache.set(cacheKey, { videos: scraped, ts: Date.now() });
