// src/lib/marketplace.ts — TS mirror of backend marketplace.js
...
export function createProduct(products: any[], data: any) {
...
export function getProduct(products: any[], id: string | number) {
  const pid = Number(id);
  return products.find(p => p.id === pid || String(p.id) === String(id)) || null;
}

export function purchaseProduct(products: any[], sales: any[], productId: string | number, buyerEmail = "buyer@alphatekx.ai") {
...
export function getSalesForSeller(sales: any[], sellerEmail?: string) {
  if (!sellerEmail) return sales;
  return sales.filter(s => s.sellerEmail === sellerEmail);
}

export function getSalesSummary(sales: any[], sellerEmail?: string) {
