// src/lib/channel.ts — Worker helper for real YouTube channel UCGm89Z31SYxEU9PEQ-p3cNA
...
export function getMockChannelInfo() {
  return {
    ...
    contentDetails: {
      relatedPlaylists: { uploads: "UU" + CHANNEL_ID.slice(2) },
    },
  };
}
...
export function getMockChannelVideos(count = 41) {
  ...
  // Ensure first is featured real video, rest varied, no exact duplicates
  return videos;
}

export async function fetchChannelInfo(apiKey?: string) {
  ...
  try {
    ...
    console.warn("[channel] fetchChannelInfo fallback to mock", e);
    return getMockChannelInfo();
  }
}

export async function fetchChannelVideos(apiKey?: string, maxResults = 50) {
  if (!apiKey) return getMockChannelVideos().slice(0, maxResults);
  // Try playlistItems via uploads playlist — most reliable for real channel videos
  try {
    ...
    if (plRes.ok) {
      ...
      if (items.length > 0) {
        const videoIds = items.map((it: any) => it.contentDetails?.videoId || it.snippet?.resourceId?.videoId).filter(Boolean).join(",");
        // Fetch details for real views/durations/thumbnails
    ...
    console.warn("[channel] playlistItems failed, trying search", e);
  }
  try {
    ...
    console.warn("[channel] fetchChannelVideos fallback to varied mock", e);
    return getMockChannelVideos().slice(0, maxResults);
  }
}
